...
async function sendToOpenClaw(imageData, context, notes) {
  ...
  if (!config.gatewayToken || !config.channelId) {
    console.error('OpenClaw Snap not configured');
    return { success: false, error: 'Not configured — open Settings' };
  }

  const md = buildMarkdown(context, notes);
  const base64Data = imageData.split(',')[1];
  const relayPort = parseInt(config.gatewayPort || '18789') + 1; // snap relay = gateway port + 1

  try {
    const resp = await fetch(`http://${config.gatewayHost || 'localhost'}:${relayPort}/snap`, {
      method: 'POST',
      headers: {
